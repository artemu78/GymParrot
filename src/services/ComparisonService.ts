...
export class ComparisonService implements IComparisonService {
  ...
  compareMovementSequence(
    ...
    try {
      ...
      const normalizedRecorded = this.normalizeSequenceLength(
        recorded,
        current.length
      );
      const normalizedCurrent = this.normalizeSequenceLength(
        current,
        recorded.length
      );
      ...
      // Generate feedback and suggestions
      const feedback = this.generateMovementFeedback(
        frameScores,
        overallScore,
        threshold
      );
      const suggestions = this.generateMovementSuggestions(
        recorded,
        current,
        difficulty
      );
  ...
  calculateSimilarityScore(
    ...
    for (let i = 0; i < minLength; i++) {
      ...
      if (
        ...
        typeof landmark1.y !== "number" ||
        typeof landmark1.z !== "number" ||
        typeof landmark2.x !== "number" ||
        typeof landmark2.y !== "number" ||
        typeof landmark2.z !== "number" ||
        isNaN(landmark1.x) ||
        isNaN(landmark1.y) ||
        isNaN(landmark1.z) ||
        isNaN(landmark2.x) ||
  ...
  private generatePoseFeedback(
    ...
    if (score >= threshold) {
      ...
      feedback.push(
        `Pose similarity: ${(score * 100).toFixed(1)}% (target: ${(threshold * 100).toFixed(1)}%)`
      );
    }
  ...
  private generateMovementFeedback(
    ...
    if (overallScore >= threshold) {
      feedback.push(
        `Excellent movement! Overall similarity: ${(overallScore * 100).toFixed(1)}%`
      );
    } else {
      feedback.push(
        `Movement similarity: ${(overallScore * 100).toFixed(1)}% (target: ${(threshold * 100).toFixed(1)}%)`
      );
    }
  ...
  private generatePoseSuggestions(
    ...
    if (difficulty === "hard") {
      suggestions.push(
        "Focus on precise positioning - small adjustments matter"
      );
    } else if (difficulty === "medium") {
      suggestions.push("Work on refining your pose - you're getting close");
    } else if (difficulty === "soft") {
      suggestions.push(
        "Get the general pose shape right - precision will come with practice"
      );
    }

    return suggestions.length > 0
      ? suggestions
      : ["Keep practicing to improve your pose accuracy"];
  }
  ...
  private generateMovementSuggestions(
    ...
    if (current.length < recorded.length * 0.8) {
      ...
      suggestions.push(
        "The movement seems too slow - try to match the target pace"
      );
    }
    ...
    suggestions.push(
      "Try to maintain consistent timing throughout the sequence"
    );
  ...
  private analyzeBodyPartDifferences(
    ...
    for (const [partName, indices] of Object.entries(bodyParts)) {
      ...
      if (validComparisons > 0) {
        ...
        if (averageDistance > 0.15) {
          // Threshold for significant difference
          feedback.push(
            `${partName.charAt(0).toUpperCase() + partName.slice(1)} positioning needs adjustment`
          );
        }
      }
    }
  ...
  private analyzeMovementPhases(frameScores: number[]): string[] {
    ...
    const beginningScore =
      frameScores.slice(0, Math.floor(length / 3)).reduce((a, b) => a + b, 0) /
      Math.floor(length / 3);
    const middleScore =
      frameScores
        .slice(Math.floor(length / 3), Math.floor((2 * length) / 3))
        .reduce((a, b) => a + b, 0) / Math.floor(length / 3);
    const endScore =
      frameScores
        .slice(Math.floor((2 * length) / 3))
        .reduce((a, b) => a + b, 0) /
  ...
  private findMajorDifferences(
    ...
    for (const [groupName, indices] of Object.entries(groups)) {
      ...
      if (validComparisons > 0) {
        ...
        differences[groupName as keyof typeof differences] =
