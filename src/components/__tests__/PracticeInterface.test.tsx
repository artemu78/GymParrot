...
describe("PracticeInterface", () => {
  ...
  describe("Pose Comparison Result", () => {
      ...
      it("should capture image and show result after countdown", async () => {
          // Use REAL timers because fake timers are flaky with mixed async/timeout flow
          vi.useRealTimers();
          ...
          // Countdown 3 (after ~200ms init delay)
          await waitFor(() => {
             expect(screen.getByText("3")).toBeInTheDocument();
          }, { timeout: 1000 });

          // Countdown 2 (after 1s)
          await waitFor(() => {
             expect(screen.getByText("2")).toBeInTheDocument();
          }, { timeout: 2000 });

          // Countdown 1 (after 1s)
          await waitFor(() => {
             expect(screen.getByText("1")).toBeInTheDocument();
          }, { timeout: 2000 });

          // After countdown finishes (1s more), result shown
          await waitFor(() => {
              expect(comparisonService.comparePoses).toHaveBeenCalled();
          }, { timeout: 2000 });

          // Should show result overlay
          await waitFor(() => {
